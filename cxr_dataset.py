...
class CPDataset(Dataset):
    ...
    def __init__(
        ...
        if fold == 'train':
            self.df = self.df[~self.df['Path'].str.contains('test')]#in column path with out test -> val as train
        else:
            self.df = self.df[self.df['Path'].str.contains('test')]#sinon -> test as val 
        ...
        self.PRED_LABEL = ['Enlarged Cardiomediastinum',
                           'Cardiomegaly',
                           'Lung Opacity',
                           'Lung Lesion',
                           'Edema',
    ...
    def __getitem__(self, idx):
        ...
        path_old = self.df.index[idx]
        path_split = path_old.split('/', 1)[1]
        print(path_old)
        print(path_old.split('/', 1)[1])
        image = Image.open(
            os.path.join(
                self.path_to_images,
                path_split))
        ...
        return (image, label, path_split) #return path
